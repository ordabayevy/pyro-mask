...
def model_1(sequences, lengths, args, batch_size=None, include_prior=True):
    ...
    with pyro.plate("sequences", num_sequences, batch_size, dim=-3) as batch:
        ...
        with pyro.plate("time", max_length, dim=-2):
            # import pdb; pdb.set_trace()
            t = torch.arange(max_length)
            with handlers.mask(mask=(t < lengths.unsqueeze(-1)).unsqueeze(-1)):
                x_prev = pyro.sample("x_prev", dist.Categorical(probs_x[0]),
                                infer={"enumerate": "parallel"})
                x_curr = pyro.sample("x_curr", dist.Categorical(probs_x[x_prev]),
                                infer={"enumerate": "parallel"})
                with tones_plate as tones:
                    pyro.sample("y", dist.Bernoulli(Vindex(probs_y)[x_curr, tones]),
                                obs=sequences[batch])
        ...
        with pyro.plate("time", max_length, dim=-2):
            ...
            with handlers.mask(mask=(t < lengths.unsqueeze(-1)).unsqueeze(-1)):
                ...
                x_curr = pyro.sample("x_curr", dist.Categorical(probs_x[x_prev]),
                                infer={"enumerate": "parallel"})
                with tones_plate as tones:
                    y_prev = torch.zeros_like(sequences[batch], dtype=torch.long)
                    y_prev[:, 1:] = sequences[batch, :-1]
                    pyro.sample("y_curr", dist.Bernoulli(Vindex(probs_y)[x_curr, y_prev, tones]),
                                obs=sequences[batch])
...
def model_3(sequences, lengths, args, batch_size=None, include_prior=True):
    ...
    with pyro.plate("sequences", num_sequences, batch_size, dim=-3) as batch:
        ...
        with pyro.plate("time", max_length, dim=-2):
            t = torch.arange(max_length)
            with handlers.mask(mask=(t < lengths.unsqueeze(-1)).unsqueeze(-1)):
                w_prev = pyro.sample("w_prev", dist.Categorical(probs_w[0]),
                                infer={"enumerate": "parallel"})
                w_curr = pyro.sample("w_curr", dist.Categorical(probs_w[w_prev]),
                                infer={"enumerate": "parallel"})
                x_prev = pyro.sample("x_prev", dist.Categorical(probs_x[0]),
                                infer={"enumerate": "parallel"})
                x_curr = pyro.sample("x_curr", dist.Categorical(probs_x[x_prev]),
                                infer={"enumerate": "parallel"})
                with tones_plate as tones:
                    pyro.sample("y_curr", dist.Bernoulli(Vindex(probs_y)[w_curr, x_curr, tones]),
                                obs=sequences[batch])
    ...
    with pyro.plate("sequences", num_sequences, batch_size, dim=-3) as batch:
        ...
        with pyro.plate("time", max_length, dim=-2):
            ...
            with handlers.mask(mask=(t < lengths.unsqueeze(-1)).unsqueeze(-1)):
                ...
                w_curr = pyro.sample("w_curr", dist.Categorical(probs_w[w_prev]),
                                infer={"enumerate": "parallel"})
                x_prev = pyro.sample("x_prev", dist.Categorical(probs_x[0, 0]),
                                infer={"enumerate": "parallel"})
                x_curr = pyro.sample("x_curr", dist.Categorical(probs_x[w_curr, x_prev]),
                                infer={"enumerate": "parallel"})
                with tones_plate as tones:
                    pyro.sample("y_curr", dist.Bernoulli(Vindex(probs_y)[w_curr, x_curr, tones]),
                                obs=sequences[batch])
...
def main(args):
    ...
    if args.print_shapes:
        first_available_dim = -2 if model is model_0 else -4
    ...
    if args.tmc:
        ...
        elbo = Elbo(max_plate_nesting=1 if model is model_0 else 3)
